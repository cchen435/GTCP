...
int chargei(gtc_bench_data_t * gtc_input)
{
	...
	int nloc_over, ipsi_in, ipsi_out, ipsi_nover_in, ipsi_nover_out,
	    igrid_in, ipsi_valid_in, ipsi_valid_out;
	int mi;
	int ntoroidal;
	int nbound;
	...
	gyrorad_const_mult = 102.0 *
	    sqrt((params->aion) * params->temperature) / (fabs(params->qion) *
							  params->b0) /
	    params->r0;
#endif
...
#pragma omp for
		for (m = 0; m < mi; m++) {
			...
			q_lc =
			    q0 + q1 * rad_frac_lc +
			    q2 * rad_frac_lc * rad_frac_lc;

			radcos_lc = 1.0 + rad_lc * cos(tdum_lc);

			pgyro_lc[2] = pgyro_lc[3] =
			    radcos_lc * gyrorad_const_mult *
			    gyrorad_const_mult * dtheta_dx;
			...
			for (larmor = 0; larmor < 4; larmor++) {
				...
				d2v =
				    _mm_load_pd(densityi_part + ij1 + mzeta +
						1);
				d3v = _mm_load_pd(densityi_part + ij2);
				d4v =
				    _mm_load_pd(densityi_part + ij2 + mzeta +
						1);
				...
				_mm_store_pd(densityi_part + ij1 + mzeta + 1,
					     res2v);
				_mm_store_pd(densityi_part + ij2, res3v);
				_mm_store_pd(densityi_part + ij2 + mzeta + 1,
					     res4v);
#endif
				...
				d2v =
				    vec_ld2(0, densityi_part + ij1 + mzeta + 1);
				d3v = vec_ld2(0, densityi_part + ij2);
				d4v =
				    vec_ld2(0, densityi_part + ij2 + mzeta + 1);
...
#pragma omp for
		for (i = 0; i < nloc_over * (mzeta + 1); i++) {
			...
			for (j = 0; j < nthreads; j++) {
				denloc_sum +=
				    densityi_local[j * (mzeta + 1) * nloc_over +
						   i];
			}
...
#pragma omp parallel for
			for (int ij = 0; ij < nloc_over; ij++) {
				...
				for (int kk = 0; kk < mzeta + 1; kk++) {
					dnitmp[ij * (mzeta + 1) + kk] =
					    densityi[ij * (mzeta + 1) + kk];
	...
	for (int i = ipsi_in; i < ipsi_out + 1; i++) {
		...
		for (int j = 0; j < mzeta + 1; j++) {
			densityi[col_start * (mzeta + 1) + j]
			    += densityi[(igrid[i] - igrid_in) * (mzeta + 1) +
					j];
		}
#if USE_GOLDRUSH
		gr_mainloop_end();
...
#pragma omp parallel for
	for (int i = ipsi_in; i < ipsi_out + 1; i++) {
		...
		for (int j = 1; j < mzeta + 1; j++) {
			densityi[j + col_start * (mzeta + 1)] =
			    densityi[j + col_end * (mzeta + 1)];
		}
#if USE_GOLDRUSH
		gr_mainloop_end();
#endif
...
int chargei_init(gtc_bench_data_t * gtc_input)
{
	...
	for (m = 0; m < mi; m++) {
		...
		for (larmor = 0; larmor < 4; larmor++) {
			...
			tdumtmp2 =
			    pi2_inv * (tflr - zetatmp * qtinv[im2]) + 10.0;
