...
static void
MatVecModM (double A[3][3], double s[3], double v[3], double m)
...
static void
MatMatModM (double A[3][3], double B[3][3], double C[3][3], double m)
...
static void
MatTwoPowModM (double A[3][3], double B[3][3], double m, long e)
...
static void
MatPowModM (double A[3][3], double B[3][3], double m, long n)
...
static int
CheckSeed (unsigned long seed[6])
{
...
void
RngStream_ParInit (int proc_num, int nprocs, int thread_num, int nthreads,
		   int nstreams, double *nextSeed_local)
{
  ...
  offset = (proc_num * nthreads * nstreams) + (thread_num * nstreams);
...
RngStream
RngStream_CreateStream (const char name[], double *nextSeed_local)
{
  int i;
  RngStream g;
  //size_t len;
...
void
RngStream_DeleteStream (RngStream g)
{
  if (g == NULL)
    return;
  if (g->name != NULL)
...
void
RngStream_ResetStartStream (RngStream g)
{
  int i;
  for (i = 0; i < 6; ++i)
...
void
RngStream_ResetNextSubstream (RngStream g)
{
  int i;
...
void
RngStream_ResetStartSubstream (RngStream g)
{
  int i;
  for (i = 0; i < 6; ++i)
    g->Cg[i] = g->Bg[i];
...
int
RngStream_SetSeed (RngStream g, unsigned long seed[6])
{
  int i;
  if (CheckSeed (seed))
    return -1;			/* FAILURE */
...
void
RngStream_AdvanceState (RngStream g, long e, long c)
{
...
void
RngStream_GetState (RngStream g, unsigned long seed[6])
{
  int i;
  for (i = 0; i < 6; ++i)
    seed[i] = g->Cg[i];
...
void
RngStream_WriteState (RngStream g)
{
  int i;
  if (g == NULL)
    return;
...
void
RngStream_WriteStateFull (RngStream g)
{
  int i;
  if (g == NULL)
    return;
...
void
RngStream_IncreasedPrecis (RngStream g, int incp)
{
  g->IncPrec = incp;
}
...
void
RngStream_SetAntithetic (RngStream g, int a)
{
  g->Anti = a;
}
...
int
RngStream_RandInt (RngStream g, int i, int j)
{
  return i + (int) ((j - i + 1.0) * RngStream_RandU01 (g));
}

unsigned int
RngStream_RandUInt (RngStream g, unsigned int i, unsigned int j)
{
