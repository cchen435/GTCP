...
#pragma omp parallel private(i,j)
		{
			...
			int jttmp, jt, kr, kp, np, nt, ij0, ipjt, ii, j1, j0,
			    ij;
...
#pragma omp for private(i, j)
			for (i = ipsi_in; i < (ipsi_out + 1); i++) {
				...
				for (j = 1; j < mtheta[i] + 1; j++) {
					...
					tgrid *= pi2_inv;
					tgrid =
					    2.0 * pi * (tgrid - ((int)tgrid));

					jttmp =
					    (int)(pi2_inv * delt[i] * tgrid +
						  0.5);
 ...
 contloop:
	...
	for (k = 0; k < mzeta; k++) {
		for (i = 0; i < nloc_over; i++) {
			...
			for (j = 0; j < nindex[k * nloc_over + i]; j++) {
				rdum +=
				    ring[k * mindex * nloc_over + i * mindex +
					 j];
			}
			if (rdum > tdum1)
				tdum1 = rdum;
	...
	if (parallel_decomp->mype == 0) {
		fprintf(stderr, "poisson solver: %d %d %f %f %d %d\n",
			max_nindex, min_nindex, tdum1, zdum, nloc_over,
			sum_nindex);
	}
...
#pragma omp parallel for
			...
			//for (int i=0; i<nloc_over; i++){
			for (int i = igrid_nover_start; i < igrid_nover_end + 1;
			     i++) {
#if USE_GOLDRUSH
				gr_mainloop_start();
#endif
				...
				//ptilde[i] = 0.0;
				int nindex_val =
				    nindex[(k - 1) * nloc_over + i] + 1;
				for (int j = 1; j < nindex_val; j++) {
					int indexp_idx =
					    (k - 1) * mindex * nloc_over +
					    i * mindex + j - 1;
