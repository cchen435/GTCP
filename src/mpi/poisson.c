...
int
poisson_initial (gtc_bench_data_t * gtc_input, int mring, int mindex,
...
#pragma omp parallel private(i,j)
	...
	real rgrid, tgrid, b, ddelr, ddelt, wght, r, t, rdum,
	  wr, tdum, wt1, wt0, rr;
	wght = 0.0;
...
#endif
	    fprintf (stderr,
		     "%d ** i=%d  ipsi_in=%d  ipsi_out=%d\n",
		     mype, i, ipsi_in, ipsi_out);
		...
		for (kr = 1; kr < mring + 1; kr++)
		  {
			...
			if (kp < 5)
			...
			else if (kp == 5)
			  {
			    ddelr =
			      0.5 *
			      (pgyro[4 * ipjt + 0] + pgyro[4 * ipjt + 2]);
			    ddelt =
			      0.5 *
			      (tgyro[4 * ipjt + 0] + tgyro[4 * ipjt + 2]);
			...
			else if (kp == 6)
			  {
			    ddelr =
			      0.5 *
			      (pgyro[4 * ipjt + 1] + pgyro[4 * ipjt + 2]);
			    ddelt =
			      0.5 *
			      (tgyro[4 * ipjt + 1] + tgyro[4 * ipjt + 2]);
			  }
			else if (kp == 7)
			  {
			    ddelr =
			      0.5 *
			      (pgyro[4 * ipjt + 1] + pgyro[4 * ipjt + 3]);
			    ddelt =
			      0.5 *
			      (tgyro[4 * ipjt + 1] + tgyro[4 * ipjt + 3]);
			  }
			else if (kp == 8)
			  {
			    ddelr =
			      0.5 *
			      (pgyro[4 * ipjt + 0] + pgyro[4 * ipjt + 3]);
			    ddelt =
			      0.5 *
			      (tgyro[4 * ipjt + 0] + tgyro[4 * ipjt + 3]);
			  }

			/* position for each point with rho_i=2.0*vring */
			r = rgrid +
			  ddelr * 2.0 * vring[kr - 1] * sqrt (0.5 / b);
			t = tgrid +
			  ddelt * 2.0 * vring[kr - 1] * sqrt (0.5 / b);
			...
			for (np = 1; np < 5; np++)
			  {
				...
				if (ij
				     ...
				     *
				     mindex
				     * nloc_over + ij0 * mindex + nt - 1])
				  {
				    ring[(k - 1) * mindex * nloc_over +
					 ij0 * mindex + nt - 1] += rr;
				    goto contloop;
				  }
			      }
			    ...
			    indexp[(k -
				    1) *
				   mindex *
				   nloc_over + ij0 * mindex + nt - 1] = ij;
			    ring[(k -
				  1) *
				 mindex *
				 nloc_over + ij0 * mindex + nt - 1] = rr;
...
int
poisson (int iflag, gtc_bench_data_t * gtc_input)
{
  ...
  int nloc_over, ipsi_in, ipsi_out, igrid_in, igrid_nover_in_radiald,
    igrid_nover_out_radiald, igrid_nover_start, igrid_nover_end,
...
#pragma omp parallel for
      ...
      for (it = 2; it < iteration + 1; it++)
	{
	  /*
	     if (parallel_decomp->mype==0) {
	     char filename[50];
	     sprintf(filename, "poisson_phi_beg.txt");
	     FILE *fp;
	     fp = fopen(filename, "w");
...
#pragma omp parallel for
	  ...
	  for (int i = igrid_nover_start; i < igrid_nover_end + 1; i++)
		...
		{
		  int indexp_idx =
		    (k - 1) * mindex * nloc_over + i * mindex + j - 1;
		  int indexp_val = indexp[indexp_idx];
		  //assert(indexp_val>=0);
		  //assert(indexp_val<nloc_over);

		  //if (indexp_val>=0&&indexp_val<nloc_over)
...
#pragma omp parallel for
	  ...
	  if (ipsi_out == mpsi)
	    {
	      for (int i = igrid[mpsi];
		   i < igrid[mpsi] + mtheta[mpsi] + 1; i++)
		{
		  phitmp[i - igrid_in] = 0.0;
		}
	    }

	  for (int i = ipsi_nover_in_radiald;
	       i < ipsi_nover_out_radiald + 1; i++)
	    {
	      phitmp[igrid[i] - igrid_in] =
		phitmp[igrid[i] + mtheta[i] - igrid_in];
	    }
	  ...
	  //if (parallel_decomp->mype==0) printf("timer2 in poisson_nop=%e\n", timer2);
	  /*
	     if (parallel_decomp->mype==0) {
	     char filename[50];
