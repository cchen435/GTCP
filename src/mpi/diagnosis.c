...
int
diagnosis (gtc_bench_data_t * gtc_input)
{
#if DIAGNOSIS
  ...
  real *xnormal =
    (real *) _mm_malloc (mflux * sizeof (real), IDEAL_ALIGNMENT);
  real *fdum = (real *) _mm_malloc ((15 + 2 * mflux) * sizeof (real),
				    IDEAL_ALIGNMENT);
  real *adum = (real *) _mm_malloc ((15 + 2 * mflux) * sizeof (real),
				    IDEAL_ALIGNMENT);
  assert (xnormal != NULL);
  assert (fdum != NULL);
  assert (adum != NULL);
  ...
  if (mype == 0)
      ...
      for (int i = 0; i < mflux; i++)
	{
	  ...
	  if (istep == ndiag)
	    printf
	      ("kappa_T at radial_bin %d is %e gyroradius=%e\n",
  ...
  MPI_Reduce (fdum + 13, adum + 13, 1, MPI_MYREAL, MPI_MAX, 0,
	      MPI_COMM_WORLD);
  MPI_Reduce (fdum + 14, adum + 14, 1, MPI_MYREAL, MPI_MIN, 0,
	      MPI_COMM_WORLD);
  MPI_Reduce (fdum + 15, adum + 15, 2 * mflux, MPI_MYREAL, MPI_SUM, 0,
	      MPI_COMM_WORLD);
#endif
  ...
  if (mype == 0)
      ...
      for (int i = 1; i < 9; i++)
	{
      ...
      printf
	("istep+mstepall=%d efield=%e eradial=%e entropyi=%e, dflowi=%e, pfluxi=%e, efluxi=%e eflux[2]=%e rmarker[2]=%e particles_energy[0]=%e particles_energy[1]=%e, sum_of_weights=%e sum_of_f0=%e max_of_f0=%e min_of_f0=%e\n",
	 istep + mstepall, fdum[0], eradial, fdum[1], fdum[3],
	 fdum[5], fdum[7], fdum[15 + mflux / 2],
	 fdum[15 + mflux + mflux / 2], fdum[9], fdum[10], fdum[11],
	 fdum[12], fdum[13], fdum[14]);
	...
	pFile = fopen ("diag_c.txt", "a");
      fprintf (pFile, "%d %e %e %e %e %e %e %e %e %e %e %e %e %e %e\n",
	       istep + mstepall, fdum[0], eradial, fdum[1], fdum[3],
	       fdum[5], fdum[7], fdum[15 + mflux / 2],
	       fdum[15 + mflux + mflux / 2], fdum[9], fdum[10],
