...
#pragma omp parallel
	{
		...
		real v, zv, xv, freq, phix, dphi, f, g, h, sp, sn, dp, dn, dpn,
		    delu, delv2;
		real zion0m, zion1m, zion2m, zion3m, zion4m, zion5m, zion03m,
		    zion04m;
		real upara, uperp2;
...
#pragma omp for
		for (m = 0; m < mi; m++) {
			...
			phix =
			    ((real) k - 10000.0 * xv) * maxwell[k - 1] +
			    (10000.0 * xv - (real) (k - 1)) * maxwell[k];
			if (xv < 0.025)
				phix =
				    4.0 / 3.0 * sqrt(xv / pi) * xv * (1.0 -
								      0.6 * xv +
								      3.0 /
								      14.0 *
								      xv * xv);
			...
			delu =
			    (zion03m - 0.5) * sqrt(12.0 * dp * freq) -
			    sp * freq;
			delv2 =
			    dpn * (zion03m - 0.5) * sqrt(12.0 / dp * freq) -
			    sn * freq + (zion04m -
					 0.5) * sqrt(12.0 * dn * freq);
			...
			dele_loc[tid * icount + mcelli] +=
			    zion4m * (delv2 + delu * (2.0 * upara + delu));
...
#pragma omp parallel for
	...
	MPI_Allreduce(marker, ddum, icount, MPI_MYREAL, MPI_SUM,
		      MPI_COMM_WORLD);
	for (int i = 0; i < icount; i++) {
...
#pragma omp parallel for
	for (int m = 0; m < mi; m++) {
		...
		real phix =
		    ((real) k - 10000.0 * xv) * maxwell[k - 1] + (10000.0 * xv -
								  (real) (k -
									  1)) *
		    maxwell[k];
		if (xv < 0.025)
			phix =
			    4.0 / 3.0 * sqrt(xv / pi) * xv * (1.0 - 0.6 * xv +
							      3.0 / 14.0 * xv *
							      xv);
		if (xv > 10.0)
			phix =
		...
		zion4m =
		    zion4m - phix / (xv * zv) * zion00m * delm[mcelli] - (phix -
									  dphi)
		    / zv * dele[mcelli];
