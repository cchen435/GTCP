...
int field(gtc_bench_data_t * gtc_input)
{
	...
	int nloc_over, ipsi_in, ipsi_out, ipsi_nover_in, ipsi_nover_out,
	    igrid_in, ipsi_in1, ipsi_out1, ipsi_nover_in_radiald,
...
#pragma omp parallel for
		for (int i = igrid_nover_in1; i < igrid_nover_out1 + 1; i++) {
			...
			real wtp11_val =
			    wtp1[2 * nloc_over * (k - 1) + 2 * ij + 0];
			real wtp12_val =
			    wtp1[2 * nloc_over * (k - 1) + 2 * ij + 1];
			int jtp11_idx =
			    jtp1[2 * nloc_over * (k - 1) + 2 * ij + 0];
			int jtp12_idx =
			    jtp1[2 * nloc_over * (k - 1) + 2 * ij + 1];
			int idx1 = (mzeta + 1) * idx1a[ij] + k;
			int idx2 = (mzeta + 1) * idx2a[ij] + k;
			evector[3 * (mzeta + 1) * ij + 3 * k]
			    = drdpa[ij] * diffr * ((1.0 - wtp11_val) *
						   phi[(jtp11_idx -
							igrid_in) * (mzeta +
								     1) + k]
						   +
		...
		for (int i = ipsi_nover_in1; i < ipsi_nover_out1 + 1; i++) {
			evector[3 * (mzeta + 1) * (igrid[i] - igrid_in) +
				3 * k] =
			    evector[3 * (mzeta + 1) *
				    (igrid[i] + mtheta[i] - igrid_in) + 3 * k];
		}
	}
...
#pragma omp parallel for
	...
	MPI_Sendrecv(sendr + igrid_nover_start1, icount, MPI_MYREAL, idest,
		     isendtag, recvl + igrid_nover_start1, icount, MPI_MYREAL,
	...
	isendtag = parallel_decomp->myrank_toroidal;
	irecvtag = isource;
	MPI_Sendrecv(sendl + igrid_nover_start1, icount, MPI_MYREAL, idest,
		     isendtag, recvr + igrid_nover_start1, icount, MPI_MYREAL,
		     isource, irecvtag, parallel_decomp->toroidal_comm,
		     &istatus2);
	...
	for (int ij = igrid_nover_start1; ij < igrid_nover_end1 + 1; ij++) {
		...
		if (mzeta == 1) {
			evector[3 * (mzeta + 1) * ij + 3 * 1 + 2] =
			    (recvr[recvr_index[ij]] -
			     recvl[recvl_index[ij]]) * diffz;
		} else {
...
#endif
		sendrs[3 * i + 0] =
		    evector[3 * (mzeta + 1) * i + 3 * mzeta + 0];
		sendrs[3 * i + 1] =
		    evector[3 * (mzeta + 1) * i + 3 * mzeta + 1];
		sendrs[3 * i + 2] =
		    evector[3 * (mzeta + 1) * i + 3 * mzeta + 2];
		recvls[3 * i + 0] = 0.0;
		recvls[3 * i + 1] = 0.0;
		recvls[3 * i + 2] = 0.0;
#if USE_GOLDRUSH
...
#pragma omp parallel for num_threads(NUM_THREADS)
		for (int i = ipsi_in1; i < ipsi_out1 + 1; i++) {
			...
			int iti = itran[i];
			for (int j = ii + 1; j < ii + iti + 1; j++) {
				evector[3 * j * (mzeta + 1) + 0] =
				    recvls[3 * (j + jt - iti) + 0];
				evector[3 * j * (mzeta + 1) + 1] =
				    recvls[3 * (j + jt - iti) + 1];
				evector[3 * j * (mzeta + 1) + 2] =
				    recvls[3 * (j + jt - iti) + 2];
			}
			for (int j = ii + iti + 1; j < ii + jt + 1; j++) {
				evector[3 * j * (mzeta + 1) + 0] =
	...
	for (int i = ipsi_in; i < ipsi_out + 1; i++) {
		...
		for (int j = 0; j < mzeta + 1; j++) {
			evector[3 * (ii - igrid_in) * (mzeta + 1) + 3 * j + 0] =
			    evector[3 * (ii + jt - igrid_in) * (mzeta + 1) +
				    3 * j + 0];
