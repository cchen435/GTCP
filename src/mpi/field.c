...
int
field (gtc_bench_data_t * gtc_input)
{
...
#pragma omp parallel for
  ...
  MPI_Sendrecv (sendr + igrid_nover_start1, icount, MPI_MYREAL, idest,
		...
		isource, irecvtag, parallel_decomp->toroidal_comm, &istatus1);
  ...
  MPI_Sendrecv (sendl + igrid_nover_start1, icount, MPI_MYREAL, idest,
		...
		isource, irecvtag, parallel_decomp->toroidal_comm, &istatus2);
  ...
  for (int ij = igrid_nover_start1; ij < igrid_nover_end1 + 1; ij++)
      ...
      if (mzeta == 1)
	{
	  evector[3 * (mzeta + 1) * ij + 3 * 1 + 2] =
	    (recvr[recvr_index[ij]] - recvl[recvl_index[ij]]) * diffz;
	}
      else
	{
