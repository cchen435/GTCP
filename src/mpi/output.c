...
int output(gtc_bench_data_t * gtc_input)
{
        ...
        MPI_Reduce(disp, disp_tmp, npartdom, MPI_INT, MPI_SUM, 0,
                   parallel_decomp->partd_comm);
        MPI_Reduce(recvsize, recvsize_tmp, npartdom, MPI_INT, MPI_SUM, 0,
                   parallel_decomp->partd_comm);

        if (myrank_partd == parallel_decomp->nproc_partd - 1) {
                if (nloc_nover != 0)
                        assert(disp[myrank_partd] + recvsize[myrank_partd] ==
                               mgrid);
        }
        /*
           if (mype==0){
...
#pragma omp parallel for
        for (int ij = igrid_nover_in; ij < igrid_nover_out + 1; ij++) {
                ...
                for (int k = 1; k < mzeta + 1; k++) {
                        eachdata_pe[(ij - igrid_nover_in) * mzeta + k - 1] =
                            phi[(ij - igrid_in) * (mzeta + 1) +
                                k] / (gyroradius * gyroradius);
                }
#if USE_GOLDRUSH
#if PHASE_LENGTH
        ...
        if (myrank_partd == 0) {
                MPI_Gather(eachdata_rad, icount, MPI_MYREAL, alldata, icount,
                           MPI_MYREAL, 0, parallel_decomp->toroidal_comm);
        }

        if (mype == 0) {

                real *alldata_ghost =
                    (real *) malloc(mgrid * mzeta * sizeof(real));
#pragma omp parallel for
                ...
                for (int i = 0; i < mzetamax - 1; i++) {
                        for (int j = 0; j < mgrid; j++) {
                                for (int k = 1; k < mzeta + 1; k++) {
                                        alldata[i * icount + j * mzeta + k -
                                                1] =
                                            0.5 *
                                            (alldata
                                             [i * icount + j * mzeta + k - 1] +
                                             alldata[(i + 1) * icount +
                                                     j * mzeta + k - 1]);
                                }
                        }
...
#endif
                        for (int k = 1; k < mzeta + 1; k++) {
                                alldata[(mzetamax - 1) * icount + j * mzeta +
                                        k - 1] =
                                    0.5 *
                                    (alldata
                                     [(mzetamax - 1) * icount + j * mzeta + k -
                                      1] + alldata_ghost[j * mzeta + k - 1]);
                        }
#if USE_GOLDRUSH
#if PHASE_LENGTH
                        gr_phase_start_s(__FILE__, __LINE__);
...
#endif
                                zdum = delz * (real) j;
                                kz = abs_min_int(mzetamax - 1,
                                                 (int)(zdum * (real) mzetamax));
...
#else
                fprintf(stderr,
                        "the output of mzeta != 0 has not been implemented\n");
