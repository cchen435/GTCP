...
extern "C" {
	...
	unsigned int RngStream_RandUInt(RngStream g, unsigned int i,
					unsigned int j);

	void RngStream_ParInit(int proc_num, int nprocs, int thread_num,
			       int nthreads, int nstreams,
			       double *nextSeed_local);
