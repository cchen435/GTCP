...
#if REMAPPING
int
igrid_bin_particles (gtc_bench_data_t * gtc_input)
{
	...
	for (i = 0; i < nloc_remap; i++)
	  {
	    for (j = 1; j < nthreads; j++)
	      {
		igrid_offsets[j * nloc_remap + i] =
		  igrid_offsets[(j - 1) * nloc_remap +
				i] + igrid_count[(j - 1) * nloc_remap + i];
	      }
	  }

	if (igrid_offsets
	    [(nthreads - 1) * nloc_remap + nloc_remap - 1] +
	    igrid_count[(nthreads - 1) * nloc_remap +
			nloc_remap - 1] != (mi - (params->holecount)))
	  {
	    fprintf (stderr, "igrid offset sum %d, mi %d\n",
		     igrid_offsets[(nthreads -
				    1) * nloc_remap +
				   nloc_remap - 1] +
		     igrid_count[(nthreads -
				  1) * nloc_remap + nloc_remap - 1], mi);
	    exit (1);
	  }
...
int
radial_bin_particles (gtc_bench_data_t * gtc_input)
{
		      ...
		      1] != (mi - (params->holecount)))
	  {
	    fprintf (stderr, "psi offset sum %d, mi %d\n",
		     psi_offsets[(nthreads - 1) * mpsi_loc +
				 mpsi_loc - 1] +
		     psi_count[(nthreads - 1) * mpsi_loc + mpsi_loc - 1], mi);
	    exit (1);
	  }
	mi_new = mi - params->holecount;
	params->mi = mi_new;
...
int
shifti_toroidal (gtc_bench_data_t * gtc_input)
{
#if USE_MPI
  gtc_global_params_t *params;
  gtc_particle_data_t *particle_data;
...
#pragma omp for
	      ...
	      if (zetaright * zetaleft > 0.0)
		{
		  ...
		  if (zetaright < 0.5)
		      ...
		      if (rpos == mrstack_max)
			{
			  ...
			  /* double the size */
			  stack_tmp =
			    (int *) malloc (2 * mrstack_max * sizeof (int));
			  assert (stack_tmp != NULL);
			  memcpy
			    (stack_tmp, rstack, mrstack_max * sizeof (int));
			  free (rstack);
			  rstack = stack_tmp;
		      ...
		      if (lpos == mlstack_max)
			{
			  ...
			  /* double the size */
			  stack_tmp =
			    (int *) malloc (2 * mlstack_max * sizeof (int));
			  assert (stack_tmp != NULL);
			  memcpy
			    (stack_tmp, lstack, mlstack_max * sizeof (int));
			  free (lstack);
			  lstack = stack_tmp;
	  ...
	  if (tid == 0)
		...
		{
		  mshift[8 * i + 3] =
		    mshift[8 * (i - 1) + 3] + mshift[8 * (i - 1) + 1];
		  mshift[8 * i + 4] =
		    mshift[8 * (i - 1) + 4] + mshift[8 * (i - 1) + 2];
		}
	      msendright =
...
#pragma omp barrier
	  ...
	  memcpy (kzi + msendright + mshift[8 * tid + 4],
		  lstack, mshift[8 * tid + 2] * sizeof (int));
	  free (lstack);
	  free (rstack);
#if USE_GOLDRUSH
#if PHASE_LENGTH
	...
	if (msendleft + msendright >= parallel_decomp->sendbuf_size)
	  {
	    fprintf (stderr,
		     ...
		     msendright, parallel_decomp->sendbuf_size);
	    exit (1);
	  }
	else
	  {
	...
	if (mrecvleft + mrecvright >= parallel_decomp->recvbuf_size)
	  {
	    fprintf (stderr,
		     ...
		     mrecvright, parallel_decomp->recvbuf_size);
	    exit (1);
	  }
...
int
shifti_radial (gtc_bench_data_t * gtc_input)
{
#if USE_MPI
  gtc_global_params_t *params;
  gtc_particle_data_t *particle_data;
