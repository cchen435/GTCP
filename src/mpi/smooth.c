...
int
smooth (int iflag, gtc_bench_data_t * gtc_input)
{
      ...
      for (int i = ipsi_in1; i < ipsi_out1 + 1; i++)
	{
	  ...
	  for (int k = 0; k < mzeta + 1; k++)
	    phitmp[(mzeta + 1) * (igrid[i] - igrid_in) +
		   k] =
	      phitmp[(mzeta + 1) * (igrid[i] + mtheta[i] - igrid_in) + k];
#if USE_GOLDRUSH
#if PHASE_LENGTH
	  gr_phase_start_s (__FILE__, __LINE__);
#else
...
#endif
	      phitmp[(mzeta + 1) * ij + k] =
		0.625 * phitmp[(mzeta + 1) * ij + k] + phism[ij];
	      phism[ij] = 0.0;
	      //  }
#if USE_GOLDRUSH
#if PHASE_LENGTH
...
#pragma omp parallel for
  ...
  if (iflag == 3)
	...
	{
	  phip00[i] =
	    phip00[i] + 0.5 * (phi00[i + 1] - phi00[i - 1]) / deltar;
	}
	...
	{
	  phi00[i] =
	    phi00[i - 1] + 0.5 * deltar * (phip00[i - 1] + phip00[i]);
	}
