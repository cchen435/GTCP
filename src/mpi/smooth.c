...
int smooth(int iflag, gtc_bench_data_t * gtc_input)
{
	...
	int nloc_over, ipsi_in, ipsi_out, igrid_in, ipsi_in1, ipsi_out1,
	    ipsi_nover_in, ipsi_nover_out, ipsi_nover_in_radiald,
	    ipsi_nover_out_radiald;
	int ipsi_nover_in1, ipsi_nover_out1, igrid_nover_in1, igrid_nover_out1,
	    igrid_nover_start1, igrid_nover_end1, igrid_over_in1,
	    igrid_over_out1, igrid_over_start1, igrid_over_end1;
	...
	if (ipsi_nover_in1 == 0)
		ipsi_nover_in1 = 1;
	if (ipsi_nover_out1 == mpsi)
		ipsi_nover_out1 = mpsi - 1;
	...
	real *phism =
	    (real *) _mm_malloc(nloc_over * sizeof(real), IDEAL_ALIGNMENT);
	assert(phism != NULL);
...
#pragma omp parallel for num_threads(NUM_THREADS)
		for (int i = ipsi_in1; i < ipsi_out1 + 1; i++) {
			...
			for (int k = 0; k < mzeta + 1; k++)
				phitmp[(mzeta + 1) * (igrid[i] - igrid_in) +
				       k] =
				    phitmp[(mzeta + 1) * (igrid[i] + mtheta[i] -
							  igrid_in) + k];
#if USE_GOLDRUSH
			gr_mainloop_end();
#endif
...
#pragma omp parallel for
			...
			for (int ij = igrid_nover_start1;
			     ij < igrid_nover_end1 + 1; ij++) {
#if USE_GOLDRUSH
			     gr_mainloop_start();
#endif
					  ...
					  1] * phitmp[(mzeta +
						      ...
						      k]) - 0.0625 * ((1.0 -
									    ...
									    *
									    ...
									    2])
								      *
								      phitmp[(mzeta + 1) * (jtp2[(k - 1) * 2 * nloc_over + ij * 2] - igrid_in) + k] + wtp2[(k - 1) * 2 * nloc_over + ij * 2] * phitmp[(mzeta + 1) * (jtp2[(k - 1) * 2 * nloc_over + ij * 2] + 1 - igrid_in) + k] + (1.0 - wtp2[(k - 1) * 2 * nloc_over + ij * 2 + 1]) * phitmp[(mzeta + 1) * (jtp2[(k - 1) * 2 * nloc_over + ij * 2 + 1] - igrid_in) + k] + wtp2[(k - 1) * 2 * nloc_over + ij * 2 + 1] * phitmp[(mzeta + 1) * (jtp2[(k - 1) * 2 * nloc_over + ij * 2 + 1] + 1 - igrid_in) + k]);
			...
			//#pragma omp parallel for
			for (int i = ipsi_nover_in1; i < ipsi_nover_out1 + 1;
			     i++) {
				phism[igrid[i] - igrid_in] =
				    phism[igrid[i] + mtheta[i] - igrid_in];
			}
...
#pragma omp for
			...
			for (int i = ipsi_nover_in1; i < ipsi_nover_out1 + 1;
				...
				phitmp[(mzeta + 1) * ij + k] =
				    ...
				    0.25 *
				    (phism[igrid[i] + mtheta[i] - igrid_in] +
				     phism[ij + 1]) - 0.0625 * (phism[igrid[i] +
								      mtheta[i]
								      - 1 -
								      igrid_in]
								+ phism[ij +
				...
				phitmp[(mzeta + 1) * ij1 + k] =
				    0.625 * phism[ij1] +
				    0.25 * (phism[ij1 - 1] + phism[ij1 + 1]) -
				    0.0625 *
				    (phism[igrid[i] + mtheta[i] - igrid_in] +
				     phism[ij1 + 2]);
				int ij2 = igrid[i] + mtheta[i] - igrid_in;
				phitmp[(mzeta + 1) * ij2 + k] =
				    0.625 * phism[ij2] +
				    0.25 * (phism[ij2 - 1] +
					    phism[igrid[i] + 1 - igrid_in]) -
				    0.0625 * (phism[ij2 - 2] +
					      phism[igrid[i] + 2 - igrid_in]);
				int ij3 = igrid[i] + mtheta[i] - 1 - igrid_in;
				phitmp[(mzeta + 1) * ij3 + k] =
				    0.625 * phism[ij3] +
...
#pragma omp parallel for
		...
		irecvtag = isource;
		MPI_Sendrecv(sendr + igrid_nover_start1, icount, MPI_MYREAL,
			     idest, isendtag, recvl + igrid_nover_start1,
...
#pragma omp parallel for
		...
		isendtag = parallel_decomp->myrank_toroidal;
		irecvtag = isource;
		MPI_Sendrecv(sendl + igrid_nover_start1, icount, MPI_MYREAL,
			     idest, isendtag, recvr + igrid_nover_start1,
			     icount, MPI_MYREAL, isource, irecvtag,
			     parallel_decomp->toroidal_comm, &istatus);
#else
...
#pragma omp parallel for
		for (int ij = igrid_nover_start1; ij < igrid_nover_end1 + 1;
		     ij++) {
#if USE_GOLDRUSH
			gr_mainloop_start();
#endif
			if (mzeta == 1) {
				phitmp[(mzeta + 1) * ij + 1] =
				    0.5 * phitmp[(mzeta + 1) * ij + 1] +
				    0.25 * (recvr[recvr_index[ij]] +
					    recvl[recvl_index[ij]]);
			} else {
				printf
				    ("mzeta!=0 hasn't been implemented in field.c\n");
			}
#if USE_GOLDRUSH
			gr_mainloop_end();
#endif
...
#pragma omp parallel for
	...
	for (int i = ipsi_in1; i < ipsi_out1 + 1; i++) {
		for (int j = 0; j < mzeta + 1; j++) {
			phitmp[(igrid[i] - igrid_in) * (mzeta + 1) + j] =
			    phitmp[(igrid[i] + mtheta[i] - igrid_in) * (mzeta +
									1) + j];
		}
	}
	...
	if (ipsi_out == mpsi) {
		for (int i = igrid[mpsi]; i < igrid[mpsi] + mtheta[mpsi] + 1;
		     i++) {
			for (int j = 0; j < mzeta + 1; j++) {
				phitmp[(i - igrid_in) * (mzeta + 1) + j] = 0.0;
			}
...
#pragma omp parallel for
	...
	if (iflag == 3) {
		...
		for (int i = 1; i < mpsi + 1; i++) {
			phi00[i] =
			    phi00[i - 1] + 0.5 * deltar * (phip00[i - 1] +
							   phip00[i]);
		}
	...
	if (iflag == 3 && idiag == 0) {
		...
		for (int i = ipsi_nover_in; i < ipsi_nover_out + 1; i++) {
			for (int j = 1 - igrid_in; j < mtheta[i] + 1 - igrid_in;
			     j++) {
				for (int k = 1; k < mzeta + 1; k++) {
					efield +=
					    phi[(igrid[i] + j) * (mzeta + 1) +
						k] * phi[(igrid[i] +
							  j) * (mzeta + 1) + k];
				}
			}
		...
		real tmp1 = 0.0;
		MPI_Allreduce(&efield, &tmp1, 1, MPI_MYREAL, MPI_SUM,
		...
		efield =
		    efield / ((real) (mzeta * sum_mtheta) * energy_unit_sq);
