...
int
gtc_mem_free (gtc_bench_data_t * gtc_input)
{
...
int
setup (gtc_bench_data_t * gtc_input)
{
...
#pragma omp parallel for
	  ...
	  if (ghost_start_global[i * nproc_radiald + myrank_radiald] != 0)
	    {
	      ...
	      nghost_start[i_loc] =
		ghost_start_global[i * nproc_radiald + myrank_radiald];
	      nghost_end[i_loc] =
		ghost_end_global[i * nproc_radiald + myrank_radiald];
	      nghost_bufsize +=
		igrid[nghost_end[i_loc]] +
		mtheta[nghost_end[i_loc]] - igrid[nghost_start[i_loc]] + 1;
	      i_loc++;
	    }
	}
    }
	  ...
	  if (ghost_start_global[myrank_radiald * nproc_radiald + i] != 0)
	    {
	      ...
	      ghost_start[i_loc] =
		ghost_start_global[myrank_radiald * nproc_radiald + i];
	      ghost_end[i_loc] =
		ghost_end_global[myrank_radiald * nproc_radiald + i];
	      ghost_bufsize +=
		igrid[ghost_end[i_loc]] +
		mtheta[ghost_end[i_loc]] - igrid[ghost_start[i_loc]] + 1;
	      i_loc++;
	    }
	}
    }
  ...
  int *igrid_count = (int *) _mm_malloc (nloc_remap * nthreads * sizeof (int),
					 IDEAL_ALIGNMENT);
  int *igrid_offsets =
  ...
  tgyro =
    (real *) _mm_malloc (4 * nloc_over * sizeof (real), IDEAL_ALIGNMENT);
  markeri =
    (real *) _mm_malloc (nloc_over * mzeta * sizeof (real), IDEAL_ALIGNMENT);
  ...
  psi_count =
    (int *) _mm_malloc (mpsi_loc * nthreads * sizeof (int), IDEAL_ALIGNMENT);
  psi_offsets =
    (int *) _mm_malloc (mpsi_loc * nthreads * sizeof (int), IDEAL_ALIGNMENT);
      ...
      for (j = 1; j < mtheta[i] + 1; j++)
	{
	  ...
	  for (k = 1; k < mzeta + 1; k++)
		...
		pow ((1.0 + r * cos (tdum)), 2);
	      if (i >= ipsi_nover_in && i <= ipsi_nover_out)
		pmarki[i] =
		  pmarki[i] + markeri[(ij - igrid_in) * mzeta + k - 1];
	    }
	}
    }
  ...
  for (i = ipsi_in; i < ipsi_out + 1; i++)
    {
      ...
      tdum =
	mi * npartdom * (rmax * rmax - rmin * rmin) / (a1 * a1 - a0 * a0);
      ...
      for (j = 1; j < mtheta[i] + 1; j++)
	{
	  ...
	  for (k = 1; k < mzeta + 1; k++)
	    {
	      markeri[(ij - igrid_in) * mzeta + k - 1] =
		tdum * markeri[(ij - igrid_in) * mzeta + k - 1] / pmarki[i];
      ...
      for (k = 1; k < mzeta + 1; k++)
	{
	  markeri[(igrid[i] - igrid_in) * mzeta + k - 1] =
	    markeri[(igrid[i] + mtheta[i] - igrid_in) * mzeta + k - 1];
	}
    }
      ...
      for (i = ipsi_in; i < ipsi_out + 1; i++)
	{
	  for (ip = 1; ip < 3; ip++)
	      ...
	      for (j = 0; j < mtheta[i] + 1; j++)
		{
		  ...
		  tdum =
		    (j * deltat[i] +
		     zdum * (qtinv[i] - qtinv[indt])) / deltat[indt];
		  jt = ((int) floor (tdum));
		  wt = tdum - ((real) jt);
...
#if OUTPUT
  real *eachdata_pe = (real *) _mm_malloc (mzeta * nloc_nover * sizeof (real),
					   IDEAL_ALIGNMENT);
  real *eachdata_rad =
  ...
  if (global_params->tauii > 0.0)
    {
      ...
      tau_vth =
	23.0 - log (sqrt (zeff * zeff * edensity0) / pow (temperature, 1.5));
      tau_vth =
