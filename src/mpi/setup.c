...
int
read_input_file(char *filename, gtc_global_params_t * global_params,
        ...
        if (mype == 0) {
                ...
                while (fgets(buf, 100, fp) != NULL) {
                        ...
                        if (strncmp(buf, "mthetamax", 9) == 0)
                                sscanf(buf, int_scan_str,
                                       &(global_params->mthetamax));

                        if (strncmp(buf, "mzetamax", 8) == 0)
                                sscanf(buf, int_scan_str,
                                       &(global_params->mzetamax));

                        if (strncmp(buf, "npe_radiald", 11) == 0)
                                sscanf(buf, int_scan_str,
                                       &(radial_decomp->npe_radiald));

                        if (strncmp(buf, "r0", 2) == 0)
                                sscanf(buf, real_scan_str,
                                       &(global_params->r0));

                        if (strncmp(buf, "nbound", 6) == 0)
                                sscanf(buf, int_scan_str,
                                       &(global_params->nbound));
...
int setup(gtc_bench_data_t * gtc_input)
{
        ...
        int irun, mstep, numberpe, mype, ntoroidal, micell, mpsi, mthetamax,
            mzetamax, npe_radiald;
        real r0;
        ...
        int nradial_dom, npartdom, particle_domain_location,
        ...
        int ipsi_nover_in_radiald, ipsi_nover_out_radiald,
            igrid_nover_in_radiald, igrid_nover_out_radiald;

        real a_in, a_out, a_nover_in, a_nover_out, a_valid_in, a_valid_out,
            rho_max;
        int *ri_pe, *ri_pe2, *tempi, *arr, *arr2;
        real *adum;

        // variables for radial domain grid ghost cell communication
        int *ghost_comm_list, *ghost_start, *ghost_end, *nghost_comm_list,
            *nghost_start, *nghost_end;
        real *ghost_sendrecvbuf, *nghost_sendrecvbuf;
        int ghost_comm_num, nghost_comm_num, ghost_bufsize, nghost_bufsize;
        int i_loc;

        // temparory variables
        int *ghost_start_local, *ghost_end_local, *ghost_start_global,
            *ghost_end_global;
        ...
        int ipsi_remap_in, ipsi_remap_out, igrid_remap_in, igrid_remap_out,
            nloc_remap;
        int remap_order;
        int mvpara, mvperp2;
        real deltavpara, deltavperp2, remap_extend;
        real *df_phase_space, *f_phase_space, *g_phase_space,
            *sendl_phase_space, *sendr_phase_space, *recvl_phase_space,
            *recvr_phase_space;
        int remapping_freq;
        ...
        global_params->mmomentsoutput = 1;
        if (irun == 0)
                global_params->mstepall = 0;
        global_params->restart_freq = 10;
#if RESTART
...
#pragma omp barrier
        ...
        if (mype == 0) {
                fprintf(stderr,
                        "Using npartdom %d, ntoroidal %d, nthreads %d\n",
        ...
        if (mype == 0)
                printf("use customized MPI mapping on BGQ\n");
        get_2d_communicator(comm2d);
#else
        if (mype == 0)
                printf("NOT use customized MPI mapping on BGQ\n");
        ...
        zetamax =
            2.0 * pi * ((real) (toroidal_domain_location + 1)) / ntoroidal;
        ...
        if (mype == 0) {
                fprintf(stderr,
                        "number of radial parition=%d number of particle copies=%d\n",
        ...
        if (left_radial_pe < 0)
                left_radial_pe = left_radial_pe + nproc_partd;
        right_radial_pe = myrank_partd + nproc_radial_partd;
        if (right_radial_pe >= nproc_partd)
                right_radial_pe = right_radial_pe - nproc_partd;

        /* Equilibrium unit: length (unit=cm) and time (unit=second) unit */
        ulength = r0;
        gyroradius =
            102.0 * sqrt(aion * temperature) / (fabs(qion) * b0) / ulength;
        ...
        deltat =
            (real *) _mm_malloc((mpsi + 1) * sizeof(real), IDEAL_ALIGNMENT);
...
#pragma omp parallel for
        ...
        if (mype == 0)
                fprintf(stderr,
                        "mi: %d, mi_total: %ld, mgrid: %d, mzetamax: %d\n", mi,
                        ((long)mi) * ((long)npartdom * ntoroidal), mgrid,
                        mzetamax);

        a_nover_in =
            sqrt((real) myrank_radiald / (real) nproc_radiald *
                 (a1 * a1 - a0 * a0) + a0 * a0);
        a_nover_out =
            sqrt((real) (myrank_radiald + 1) / (real) nproc_radiald *
                 (a1 * a1 - a0 * a0) + a0 * a0);
        ...
        for (i = ipsi_valid_in; i < ipsi_valid_out + 1; i++) {
                if (ri_pe[i] == myrank_partd && ipsi_nover_in == -1)
                        ipsi_nover_in = i;
                if (ri_pe[i] != myrank_partd && ipsi_nover_in != -1
                    && ipsi_nover_out == -1)
                        ipsi_nover_out = i - 1;
        ...
        for (i = 0; i < mpsi + 1; i++) {
                if (arr2[i] != 1) {
                        printf
                            ("failed radial partition: use more radial grid cells (or less procs)");
                        MPI_Abort(MPI_COMM_WORLD, 1);
                }
        }
        ...
        if (mype == 0)
                printf
                    ("Using n_rad_buf=%d buffer cells: gyroradius/deltar=%f\n",
        ...
        if (ipsi_in < 0 || ipsi_out > mpsi) {
                printf
                    ("mype=%d: in set_radial_decomp:ipsi_in < 0 or ipsi_out > mpsi\n",
                     mype);
                MPI_Abort(MPI_COMM_WORLD, 1);
        }
        ...
        ghost_start_local =
            (int *)_mm_malloc(nproc_radiald * sizeof(int), IDEAL_ALIGNMENT);
        ghost_end_local =
            (int *)_mm_malloc(nproc_radiald * sizeof(int), IDEAL_ALIGNMENT);
        ghost_start_global =
            (int *)_mm_malloc(nproc_radiald * nproc_radiald * sizeof(int),
                              IDEAL_ALIGNMENT);
        ghost_end_global =
            (int *)_mm_malloc(nproc_radiald * nproc_radiald * sizeof(int),
                              IDEAL_ALIGNMENT);
        ...
        for (i = ipsi_in; i < ipsi_out + 1; i++) {
                ...
                if (proc_num != myrank_radiald
                    && ghost_start_local[proc_num] ==
                    0 & ghost_end_local[proc_num] == 0) {
        ...
        MPI_Allgather(ghost_start_local, nproc_radiald, MPI_INT,
                      ghost_start_global, nproc_radiald, MPI_INT,
                      *radiald_comm);
        MPI_Allgather(ghost_end_local, nproc_radiald, MPI_INT, ghost_end_global,
                      nproc_radiald, MPI_INT, *radiald_comm);

        nghost_comm_num = 0;
        for (i = 0; i < nproc_radiald; i++) {
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [i * nproc_radiald + myrank_radiald] != 0) {
                                nghost_comm_num += 1;
                        }
                }
        }
        ...
        nghost_comm_list =
            (int *)_mm_malloc(nghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
        nghost_start =
            (int *)_mm_malloc(nghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
        nghost_end =
            (int *)_mm_malloc(nghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [i * nproc_radiald + myrank_radiald] != 0) {
                                ...
                                nghost_start[i_loc] =
                                    ghost_start_global[i * nproc_radiald +
                                                       myrank_radiald];
                                nghost_end[i_loc] =
                                    ghost_end_global[i * nproc_radiald +
                                                     myrank_radiald];
                                nghost_bufsize +=
                                    igrid[nghost_end[i_loc]] +
                                    mtheta[nghost_end[i_loc]] -
                                    igrid[nghost_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        nghost_sendrecvbuf =
            (real *) _mm_malloc(nghost_bufsize * (mzeta + 1) * 3 * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        for (i = 0; i < nproc_radiald; i++) {
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [myrank_radiald * nproc_radiald + i] != 0) {
                                ghost_comm_num++;
                        }
                }
        }
        ...
        ghost_comm_list =
            (int *)_mm_malloc(ghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
        ghost_start =
            (int *)_mm_malloc(ghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
        ghost_end =
            (int *)_mm_malloc(ghost_comm_num * sizeof(int), IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [myrank_radiald * nproc_radiald + i] != 0) {
                                ...
                                ghost_start[i_loc] =
                                    ghost_start_global[myrank_radiald *
                                                       nproc_radiald + i];
                                ghost_end[i_loc] =
                                    ghost_end_global[myrank_radiald *
                                                     nproc_radiald + i];
                                ghost_bufsize +=
                                    igrid[ghost_end[i_loc]] +
                                    mtheta[ghost_end[i_loc]] -
                                    igrid[ghost_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        ghost_sendrecvbuf =
            (real *) _mm_malloc(ghost_bufsize * (mzeta + 1) * 3 * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        for (i = ipsi_valid_in; i < ipsi_valid_out + 1; i++) {
                if (ri_pe2[i] == myrank_radiald && ipsi_nover_in_radiald == -1)
                        ipsi_nover_in_radiald = i;
                if (ri_pe2[i] != myrank_radiald && ipsi_nover_in_radiald != -1
                    && ipsi_nover_out_radiald == -1)
                        ipsi_nover_out_radiald = i - 1;
        }

        if (ipsi_nover_out_radiald == -1)
                ipsi_nover_out_radiald = ipsi_valid_out;
        if (ipsi_nover_in_radiald == -1) {
                ipsi_nover_out_radiald = 0;
                ipsi_nover_in_radiald = 1;
        }
        igrid_nover_in_radiald = igrid[ipsi_nover_in_radiald];
        igrid_nover_out_radiald =
            igrid[ipsi_nover_out_radiald] + mtheta[ipsi_nover_out_radiald];
        ...
        df_phase_space =
            (real *) _mm_malloc(nloc_remap * (mzeta + 1) * (mvpara + 1) *
                                (mvperp2 + 1) * sizeof(real), IDEAL_ALIGNMENT);
        f_phase_space =
            (real *) _mm_malloc(nloc_remap * (mzeta + 1) * (mvpara + 1) *
                                (mvperp2 + 1) * sizeof(real), IDEAL_ALIGNMENT);
        g_phase_space =
            (real *) _mm_malloc(nloc_remap * (mzeta + 1) * (mvpara + 1) *
                                (mvperp2 + 1) * sizeof(real), IDEAL_ALIGNMENT);
        sendl_phase_space =
            (real *) _mm_malloc(nloc_remap * (mvpara + 1) * (mvperp2 + 1) *
                                sizeof(real), IDEAL_ALIGNMENT);
        sendr_phase_space =
            (real *) _mm_malloc(nloc_remap * (mvpara + 1) * (mvperp2 + 1) *
                                sizeof(real), IDEAL_ALIGNMENT);
        recvl_phase_space =
        ...
        int *nghost_remap_comm_list =
            (int *)_mm_malloc(nghost_remap_comm_num * sizeof(int),
                              IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [i * nproc_radiald + myrank_radiald] != 0) {
                                ...
                                nghost_remap_start[i_loc] =
                                    ghost_start_global[i * nproc_radiald +
                                                       myrank_radiald];
                                nghost_remap_end[i_loc] =
                                    ghost_end_global[i * nproc_radiald +
                                                     myrank_radiald];
                                nghost_remap_bufsize +=
                                    igrid[nghost_remap_end[i_loc]] +
                                    mtheta[nghost_remap_end[i_loc]] -
                                    igrid[nghost_remap_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        real *nghost_remap_sendrecvbuf =
            (real *) _mm_malloc(nghost_remap_bufsize * (mzeta + 1) *
                                (mvpara + 1) * (mvperp2 + 1) * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        int *ghost_remap_comm_list =
            (int *)_mm_malloc(ghost_remap_comm_num * sizeof(int),
                              IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [myrank_radiald * nproc_radiald + i] != 0) {
                                ...
                                ghost_remap_start[i_loc] =
                                    ghost_start_global[myrank_radiald *
                                                       nproc_radiald + i];
                                ghost_remap_end[i_loc] =
                                    ghost_end_global[myrank_radiald *
                                                     nproc_radiald + i];
                                ghost_remap_bufsize +=
                                    igrid[ghost_remap_end[i_loc]] +
                                    mtheta[ghost_remap_end[i_loc]] -
                                    igrid[ghost_remap_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        real *ghost_remap_sendrecvbuf =
            (real *) _mm_malloc(ghost_remap_bufsize * (mzeta + 1) *
                                (mvpara + 1) * (mvperp2 + 1) * sizeof(real),
                                IDEAL_ALIGNMENT);

        int *igrid_count =
            (int *)_mm_malloc(nloc_remap * nthreads * sizeof(int),
                              IDEAL_ALIGNMENT);
        int *igrid_offsets =
            (int *)_mm_malloc(nloc_remap * nthreads * sizeof(int),
                              IDEAL_ALIGNMENT);
        ...
        pgyro =
            (real *) _mm_malloc(4 * nloc_over * sizeof(real), IDEAL_ALIGNMENT);
        tgyro =
            (real *) _mm_malloc(4 * nloc_over * sizeof(real), IDEAL_ALIGNMENT);
        markeri =
            (real *) _mm_malloc(nloc_over * mzeta * sizeof(real),
                                IDEAL_ALIGNMENT);

        densityi =
            (wreal *) _mm_malloc((mzeta + 1) * nloc_over * sizeof(wreal),
                                 IDEAL_ALIGNMENT);
        densityi_local =
            (wreal *) _mm_malloc((mzeta + 1) * nloc_over * nthreads *
                                 sizeof(wreal), IDEAL_ALIGNMENT);

        dnitmp =
            (wreal *) _mm_malloc((mzeta + 1) * nloc_over * sizeof(wreal),
                                 IDEAL_ALIGNMENT);
        recvr =
            (wreal *) _mm_malloc(nloc_over * sizeof(wreal), IDEAL_ALIGNMENT);
        sendl =
            (wreal *) _mm_malloc(nloc_over * sizeof(wreal), IDEAL_ALIGNMENT);
        ...
        psi_count =
            (int *)_mm_malloc(mpsi_loc * nthreads * sizeof(int),
                              IDEAL_ALIGNMENT);
        psi_offsets =
            (int *)_mm_malloc(mpsi_loc * nthreads * sizeof(int),
                              IDEAL_ALIGNMENT);
        ...
        phi =
            (real *) _mm_malloc((mzeta + 1) * nloc_over * sizeof(real),
                                IDEAL_ALIGNMENT);
        evector =
            (real *) _mm_malloc(3 * (mzeta + 1) * nloc_over * sizeof(real),
                                IDEAL_ALIGNMENT);

        jtp1 =
            (int *)_mm_malloc(2 * nloc_over * mzeta * sizeof(int),
                              IDEAL_ALIGNMENT);
        jtp2 =
            (int *)_mm_malloc(2 * nloc_over * mzeta * sizeof(int),
                              IDEAL_ALIGNMENT);
        wtp1 =
            (real *) _mm_malloc(2 * nloc_over * mzeta * sizeof(real),
                                IDEAL_ALIGNMENT);
        wtp2 =
            (real *) _mm_malloc(2 * nloc_over * mzeta * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        recvl_index =
            (int *)_mm_malloc(nloc_over * sizeof(int), IDEAL_ALIGNMENT);
        recvr_index =
            (int *)_mm_malloc(nloc_over * sizeof(int), IDEAL_ALIGNMENT);
        ...
        sendrsf =
            (real *) _mm_malloc(3 * nloc_over * sizeof(real), IDEAL_ALIGNMENT);
        recvlsf =
            (real *) _mm_malloc(3 * nloc_over * sizeof(real), IDEAL_ALIGNMENT);
        ...
        phitmps =
            (real *) _mm_malloc(nloc_over * (mzeta + 1) * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        tmp =
            (real *) _mm_malloc(4 * neop * neot * neoz * sizeof(real),
                                IDEAL_ALIGNMENT);
        tmp_loc =
            (real *) _mm_malloc(nthreads * 3 * neop * neot * neoz *
                                sizeof(real), IDEAL_ALIGNMENT);
...
#pragma omp parallel for private(i)
        ...
        for (i = ipsi_in; i < ipsi_out + 1; i++) {
                ...
                for (j = 1; j < mtheta[i] + 1; j++) {
                        ...
                        for (k = 1; k < mzeta + 1; k++) {
                                ...
                                markeri[(ij - igrid_in) * mzeta + k - 1] =
                                    pow((1.0 + r * cos(tdum)), 2);
                                if (i >= ipsi_nover_in && i <= ipsi_nover_out)
                                        pmarki[i] =
                                            pmarki[i] +
                                            markeri[(ij - igrid_in) * mzeta +
                                                    k - 1];
                        }
                }
        }
        ...
        for (i = ipsi_in; i < ipsi_out + 1; i++) {
                ...
                tdum =
                    mi * npartdom * (rmax * rmax - rmin * rmin) / (a1 * a1 -
                                                                   a0 * a0);
                ...
                for (j = 1; j < mtheta[i] + 1; j++) {
                        ...
                        for (k = 1; k < mzeta + 1; k++) {
                                markeri[(ij - igrid_in) * mzeta + k - 1] =
                                    tdum * markeri[(ij - igrid_in) * mzeta + k -
                                                   1] / pmarki[i];
                ...
                for (k = 1; k < mzeta + 1; k++) {
                        markeri[(igrid[i] - igrid_in) * mzeta + k - 1] =
                            markeri[(igrid[i] + mtheta[i] - igrid_in) * mzeta +
                                    k - 1];
                }
        }
        ...
        for (int i = ipsi_in; i < ipsi_out + 1; i++) {
                ...
                if (myrank_toroidal == 0) {
                        for (int j = 0; j < jt + 1; j++) {
                                recvr_index[ii + j - igrid_in] =
                                    ii + j - igrid_in;
                        }
                        for (int j = ii; j < ii + iti + 1; j++) {
                                recvl_index[j - igrid_in] =
                                    j + jt - iti - igrid_in;
                        }
                ...
                for (i = ipsi_in; i < ipsi_out + 1; i++) {
                        for (ip = 1; ip < 3; ip++) {
                                ...
                                for (j = 0; j < mtheta[i] + 1; j++) {
                                        ...
                                        if (ip == 1) {
                                                wtp1[(k - 1) * 2 * nloc_over +
                                                     (ij - igrid_in) * 2] = wt;
                                                jtp1[(k - 1) * 2 * nloc_over +
                                                     (ij - igrid_in) * 2] =
                                                    igrid[indp] + jt;
                                        } else {
                                                wtp2[(k - 1) * 2 * nloc_over +
                                                     (ij - igrid_in) * 2] = wt;
                                                jtp2[(k - 1) * 2 * nloc_over +
                                                     (ij - igrid_in) * 2] =
                                                    igrid[indp] + jt;
                                        }
        ...
        flux_data =
            (real *) _mm_malloc(6 * mflux * sizeof(real), IDEAL_ALIGNMENT);
        amp_mode =
            (real *) _mm_malloc(4 * num_mode * sizeof(real), IDEAL_ALIGNMENT);
        eigenmode =
            (real *) _mm_malloc(num_mode * m_poloidal * mpsi * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        int nloc_calc_moments =
            igrid[ipsi_moments_out] + mtheta[ipsi_moments_out] -
            igrid[ipsi_moments_in] + 1;
        int igrid_moments_in = igrid[ipsi_moments_in];

        real *moments =
            (real *) _mm_malloc(nthreads * 7 * (mzeta + 1) * nloc_calc_moments *
                                sizeof(real), IDEAL_ALIGNMENT);
        real *momentstmp =
            (real *) _mm_malloc(7 * (mzeta + 1) * nloc_calc_moments *
                                sizeof(real), IDEAL_ALIGNMENT);
        real *sendl_moments =
            (real *) _mm_malloc(7 * nloc_calc_moments * sizeof(real),
                                IDEAL_ALIGNMENT);
        real *recvr_moments =
            (real *) _mm_malloc(7 * nloc_calc_moments * sizeof(real),
                                IDEAL_ALIGNMENT);
        ...
        int *nghost_moments_comm_list =
            (int *)_mm_malloc(nghost_moments_comm_num * sizeof(int),
                              IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [i * nproc_radiald + myrank_radiald] != 0) {
                                ...
                                nghost_moments_start[i_loc] =
                                    ghost_start_global[i * nproc_radiald +
                                                       myrank_radiald];
                                nghost_moments_end[i_loc] =
                                    ghost_end_global[i * nproc_radiald +
                                                     myrank_radiald];
                                nghost_moments_bufsize +=
                                    igrid[nghost_moments_end[i_loc]] +
                                    mtheta[nghost_moments_end[i_loc]] -
                                    igrid[nghost_moments_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        real *nghost_moments_sendrecvbuf =
            (real *) _mm_malloc(nghost_moments_bufsize * (mzeta + 1) * 7 *
                                sizeof(real), IDEAL_ALIGNMENT);
        ...
        int *ghost_moments_comm_list =
            (int *)_mm_malloc(ghost_moments_comm_num * sizeof(int),
                              IDEAL_ALIGNMENT);
                ...
                if (i != myrank_radiald) {
                        if (ghost_start_global
                            [myrank_radiald * nproc_radiald + i] != 0) {
                                ...
                                ghost_moments_start[i_loc] =
                                    ghost_start_global[myrank_radiald *
                                                       nproc_radiald + i];
                                ghost_moments_end[i_loc] =
                                    ghost_end_global[myrank_radiald *
                                                     nproc_radiald + i];
                                ghost_moments_bufsize +=
                                    igrid[ghost_moments_end[i_loc]] +
                                    mtheta[ghost_moments_end[i_loc]] -
                                    igrid[ghost_moments_start[i_loc]] + 1;
                                i_loc++;
                        }
                }
        }
        ...
        real *ghost_moments_sendrecvbuf =
            (real *) _mm_malloc(ghost_moments_bufsize * (mzeta + 1) * 7 *
                                sizeof(real), IDEAL_ALIGNMENT);
#endif

#if OUTPUT
        real *eachdata_pe =
            (real *) _mm_malloc(mzeta * nloc_nover * sizeof(real),
                                IDEAL_ALIGNMENT);
        real *eachdata_rad =
            (real *) _mm_malloc(mzeta * mgrid * sizeof(real), IDEAL_ALIGNMENT);
        real *alldata =
            (real *) _mm_malloc(mzetamax * mzeta * mgrid * sizeof(real),
                                IDEAL_ALIGNMENT);
        real *psizeta =
            (real *) _mm_malloc(mtdiag * (mpsi / 2 - 8) * sizeof(real),
                                IDEAL_ALIGNMENT);
#endif
        ...
        assert(ghost_moments_comm_list != NULL);
        assert(ghost_moments_start != NULL);
        assert(ghost_moments_end != NULL);
        assert(ghost_moments_sendrecvbuf != NULL);
        assert(nghost_moments_comm_list != NULL);
        assert(nghost_moments_start != NULL);
        assert(nghost_moments_end != NULL);
...
#pragma omp parallel for private(i)
        ...
        recvbuf_size = (mi / 5);
        recvbuf =
            (real *) _mm_malloc(recvbuf_size * nparam * sizeof(real),
                                IDEAL_ALIGNMENT);
        assert(recvbuf != NULL);
        sendbuf_size = (mi / 5);
        sendbuf =
            (real *) _mm_malloc(sendbuf_size * nparam * sizeof(real),
                                IDEAL_ALIGNMENT);
        assert(sendbuf != NULL);
#endif
        ...
        if (global_params->tauii > 0.0) {
                ...
                tau_vth =
                    23.0 -
                    log(sqrt(zeff * zeff * edensity0) / pow(temperature, 1.5));
                tau_vth =
                    2.09e7 * pow(temperature,
                                 1.5) * sqrt(aion) / (edensity0 * tau_vth *
                                                      utime * zeff);
                global_params->tauii = tau_vth * global_params->tauii;
                if (mype == 0)
                        printf("collision time tauii=%e nu_star = %e q=%e\n",
                               global_params->tauii,
                               q / (global_params->tauii * gyroradius *
                                    pow(r, 1.5)), q);
        }
        dele = tmp;
...
//#if USE_MPI
        ...
        radial_decomp->radial_part_domain_location =
            radial_part_domain_location;
