...
int pushi(gtc_bench_data_t * gtc_input)
{
 ...
 /*******/
        ...
        real *hfluxpsi_all =
            (real *) _mm_malloc(((mpsi + 1) + 6 * mflux) * sizeof(real),
                                IDEAL_ALIGNMENT);
...
#pragma omp parallel for schedule(static)
        ...
        /*  update GC position  */
#pragma omp parallel            // CHECK THE END OF THIS
        {
                ...
                real r, rinv, tem, q, qinv, cost, sint, b, g, gp, ri,
                    ...
                    dptdp, dptdt, dptdz,
                    epara, vdr, wdrive, wdrive2, wdrive3, wpara, wdrift, wdot,
                    pdot, tdot, zdot, rdot;
                ...
                real vdrenergy;
                real *rmarker_temp =
                    (real *) _mm_malloc((5 * mflux + (mpsi + 1)) * sizeof(real),
                                        IDEAL_ALIGNMENT);
                ...
                for (m = 0; m < mi; m++) {
                        ...
                        for (larmor = 0; larmor < 4; larmor++) {
                                ...
                                if (fabs(rhotmp) > rho_max) {
                                        printf
                                            ("warning: reducing rhoi to %e from %e\n",
                                             (rhotmp / fabs(rhotmp)) * rho_max /
                                             pgyro_lc[larmor], rhoi);
                                        rhotmp =
                                            (rhotmp / fabs(rhotmp)) * rho_max;
                                        rhoi = rhotmp / pgyro_lc[larmor];
                                }
                                rdum =
                                    delr * abs_min_real(a_diff,
                                                        r_diff + rhotmp);
                                ...
                                tdumtmp =
                                    pi2_inv * (tflr - zetatmp * qtinv[im]) +
                                    10.0;
                                tdumtmp2 =
                                    pi2_inv * (tflr - zetatmp * qtinv[im2]) +
                                    10.0;
                                ...
                                e1 = e1 + wpt000 * (wz0 * evector10 +
                                                    wz1 * evector13)
                                    + wpt010 * (wz0 * evector20 +
                                                wz1 * evector23)
                                    + wpt101 * (wz0 * evector30 +
                                                wz1 * evector33)
                                    + wpt111 * (wz0 * evector40 +
                                                wz1 * evector43);

                                e2 = e2 + wpt000 * (wz0 * evector11 +
                                                    wz1 * evector14)
                                    + wpt010 * (wz0 * evector21 +
                                                wz1 * evector24)
                                    + wpt101 * (wz0 * evector31 +
                                                wz1 * evector34)
                                    + wpt111 * (wz0 * evector41 +
                                                wz1 * evector44);

                                e3 = e3 + wpt000 * (wz0 * evector12 +
                                                    wz1 * evector15)
                                    + wpt010 * (wz0 * evector22 +
                                                wz1 * evector25)
                                    + wpt101 * (wz0 * evector32 +
                                                wz1 * evector35)
                                    + wpt111 * (wz0 * evector42 +
                                                wz1 * evector45);
#endif
                        ...
                        deni =
                            1.0 / (g * q + ri + zion3m * (g * rip - ri * gp));
                        upara = zion3m * b * cmratio;
                        energy =
                            0.5 * aion * upara * upara + zion5m * zion5m * b;
                        ...
                        wdrift =
                            q * (g * dbdt * dptdp - g * dbdp * dptdt +
                                 ri * dbdp * dptdz) * deni * dedb * qion * tem;
                        ...
                        wdrive3 =
                            q * (-g * dptdp * dbdt + g * dptdt * dbdp -
                                 ri * dptdz * dbdp) * deni * zion5m * zion5m *
                            tem;
                        ...
                        cdot = -z05[m] * (wpara + wdrift - wdrive3);
#else
                        /* two weights scheme by W. W. Lee etc. CPC, 2010 */
                        wdot = (z05[m] - zion4m) * (wdrive + wpara + wdrift);
                        //wdrive2 = (gp*dptdt - q*rip*dptdz) * deni * zion5m * zion5m * b* tem;
                        //wdrive2 = q *(-g*dedb*dbdt)* deni;
                        wdrive3 =
                            q * (-g * dptdp * dbdt + g * dptdt * dbdp -
                                 ri * dptdz * dbdp) * deni * zion5m * zion5m *
                            tem;
                        cdot = (z05[m] - zion4m) * (wdrive + wdrive3);
#endif
#else
                        wdot =
                            (z05[m] - paranl * zion4m) * (wdrive + wpara +
                                                          wdrift);
#endif

                        /* self-consistent and external electric field for marker orbits */
                        dptdp =
                            dptdp * nonlinear + gyroradius * (flow0 +
                                                              flow1 * r * ainv +
                                                              flow2 * r * r *
                                                              ainv * ainv);
                        ...
                        /* particle velocity */
                        pdot =
                            q * (-g * dedb * dbdt - g * dptdt +
                                 ri * dptdz) * deni;
                        tdot =
                            (upara * b * (1.0 - q * gp * zion3m) +
                             q * g * (dedb * dbdp + dptdp)) * deni;
                        zdot =
                            (upara * b * q * (1.0 + rip * zion3m) -
                             q * ri * (dedb * dbdp + dptdp)) * deni;
                        rdot =
                            ((gp * zion3m - 1.0) * (dedb * dbdt +
                                                    paranl * dptdt) -
                             paranl * q * (1.0 + rip * zion3m) * dptdz) * deni;

                        /* update particle position */
#if SQRT_PRECOMPUTED
                        zion0m =
                            max(1.0e-8 * psimax,
                                0.5 * zion00m * zion00m + dtime * pdot);
...
#pragma omp critical
                {
                        ...
                        if (idiag == 0) {
                                ...
                                scalar_data[17] =
                                    max(scalar_data[17], max_of_f0);
                                scalar_data[18] =
                                    min(scalar_data[18], min_of_f0);
                        }
        ...
        if (idiag == 0) {
                MPI_Allreduce(hfluxpsi, hfluxpsi_all, mpsi + 1, MPI_MYREAL,
                              MPI_SUM, MPI_COMM_WORLD);
                for (int i = 0; i < mpsi + 1; i++) {
                        hfluxpsi[i] = hfluxpsi_all[i] * pmarki[i];
                }

                MPI_Allreduce(dmark, dmark_all, mflux, MPI_MYREAL, MPI_SUM,
                              MPI_COMM_WORLD);
                MPI_Allreduce(dden, dden_all, mflux, MPI_MYREAL, MPI_SUM,
                              MPI_COMM_WORLD);
                for (int i = 0; i < mflux; i++) {
                        dmark[i] = dmark_all[i] / max(1.0, dden_all[i]);
                        pfluxpsi[i] =
                            (1.0 - tdum) * pfluxpsi[i] + tdum * dmark[i];
                }
        }
        ...
        if (nonlinear > 0.5 && paranl < 0.5) {
                ...
                if (istep % ndiag == 0) {
                        if (irk == 2) {
                                MPI_Allreduce(dden, dden_all, mflux, MPI_MYREAL,
                                              MPI_SUM, MPI_COMM_WORLD);
                                MPI_Allreduce(dtem, dtem_all, mflux, MPI_MYREAL,
                                              MPI_SUM, MPI_COMM_WORLD);
                                for (int i = 0; i < mflux; i++) {
                                        dtem[i] =
                                            dtem_all[i] * tem_inv / max(1.0,
                                                                        dden_all
                                                                        [i]);
                                        rdtemi[i] =
                                            (1.0 - tdum) * rdtemi[i] +
...
#endif
                                        int m, ip;
                                        real zion0m, zion1m, zion2m, zion3m,
                                            zion4m, zion5m;
