...
int
pushi (gtc_bench_data_t * gtc_input)
{
...
#pragma omp for schedule(static) nowait
	...
	wdrive3 =
	  q * (-g * dptdp * dbdt + g * dptdt * dbdp -
	       ri * dptdz * dbdp) * deni * zion5m * zion5m * tem;
...
#if SQRT_PRECOMPUTED
	zion0m =
	  max (1.0e-8 * psimax, 0.5 * zion00m * zion00m + dtime * pdot);
	...
	if (idiag == 0)
	  {
	    ...
	    vdrenergy =
	      vdr * rinv * (energy -
			    1.5 * aion * vthi * vthi * rtemi[ii]) * zion04m;
