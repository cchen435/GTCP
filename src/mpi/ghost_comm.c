...
#pragma omp parallel for
		for (int j = 0; j < count; j++) {
			...
			gr_mainloop_start();
#endif
			moments_data[j + offset2] +=
			    nghost_sendrecvbuf[j + offset];
#if USE_GOLDRUSH
			gr_mainloop_end();
#endif
		}
...
int
fix_radial_ghosts(gtc_bench_data_t * gtc_input, real * data, int mzeta, int dim)
{
	gtc_global_params_t *params;
	gtc_field_data_t *field_data;
	gtc_radial_decomp_t *radial_decomp;
