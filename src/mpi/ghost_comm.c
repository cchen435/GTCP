...
#if CALC_MOMENTS
int
sum_plane_moments (gtc_bench_data_t * gtc_input)
{
  ...
  MPI_Request *recv_request =
    (MPI_Request *) malloc (nghost_comm_num * sizeof (MPI_Request));
  MPI_Status *recv_status =
    (MPI_Status *) malloc (nghost_comm_num * sizeof (MPI_Status));

  MPI_Request *send_request =
    (MPI_Request *) malloc (ghost_comm_num * sizeof (MPI_Request));
  MPI_Status *send_status =
    (MPI_Status *) malloc (ghost_comm_num * sizeof (MPI_Status));
  ...
  for (int i = 0; i < nghost_comm_num; i++)
    {
      ...
      ipsi_e = nghost_end[i];
      count =
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 +
								mzeta) * 7;
      MPI_Irecv (nghost_sendrecvbuf + offset, count, MPI_MYREAL,
		 proc_num, proc_num, radial_decomp->radiald_comm,
		 &recv_request[i]);
      offset += count;
    }
  ...
  for (int i = 0; i < ghost_comm_num; i++)
	...
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 +
								mzeta) * 7;
      MPI_Isend (moments_data + offset, count, MPI_MYREAL, proc_num,
		 radial_decomp->myrank_radiald,
...
#pragma omp parallel for
  ...
  for (int i = 0; i < ghost_comm_num; i++)
	...
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 +
								mzeta) * 7;
      MPI_Irecv (ghost_sendrecvbuf + offset, count, MPI_MYREAL,
		 proc_num, proc_num, radial_decomp->radiald_comm,
		 &recv_request1[i]);
      offset += count;
    }
...
#if REMAPPING
int
sum_plane_remap (gtc_bench_data_t * gtc_input, int type)
{
  gtc_global_params_t *params;
  ...
  assert (offset ==
	  nghost_bufsize * (mzeta + 1) * (mvpara + 1) * (mvperp2 + 1));
  ...
  for (int i = 0; i < ghost_comm_num; i++)
    {
      ...
      ipsi_e = ghost_end[i];
      offset =
	(igrid[ipsi_s] - igrid_remap_in) * (1 + mzeta) * (mvpara +
							  1) * (mvperp2 + 1);
	...
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 +
								mzeta)
	* (mvpara + 1) * (mvperp2 + 1);
      MPI_Isend (phase_space_data + offset, count, MPI_MYREAL,
		 proc_num, radial_decomp->myrank_radiald,
		 radial_decomp->radiald_comm, &send_request[i]);
...
#pragma omp parallel for
  ...
  assert (offset ==
	  ghost_bufsize * (mzeta + 1) * (mvpara + 1) * (mvperp2 + 1));
...
int
sum_plane (gtc_bench_data_t * gtc_input)
{
  gtc_global_params_t *params;
  gtc_field_data_t *field_data;
  gtc_radial_decomp_t *radial_decomp;
  ...
  for (int i = 0; i < nghost_comm_num; i++)
    {
      ...
      count =
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 + mzeta);
  ...
  for (int i = 0; i < ghost_comm_num; i++)
    {
      ...
      count =
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 + mzeta);
      MPI_Isend (densityi + offset, count, MPI_MYREAL, proc_num,
		 radial_decomp->myrank_radiald,
		 radial_decomp->radiald_comm, &send_request[i]);
    }
...
int
fix_radial_ghosts (gtc_bench_data_t * gtc_input, real * data, int mzeta,
		   int dim)
{
  gtc_global_params_t *params;
  gtc_field_data_t *field_data;
  ...
  for (int i = 0; i < nghost_comm_num; i++)
	...
	(igrid[ipsi_e] + mtheta[ipsi_e] - igrid[ipsi_s] + 1) * (1 +
      ...
      MPI_Isend (data + offset, count, MPI_MYREAL, proc_num,
		 radial_decomp->myrank_radiald,
		 radial_decomp->radiald_comm, &send_request[i]);
    }
